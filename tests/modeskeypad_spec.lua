...
describe('ModesKeypadSpec', function()
  ...
  it('given a selection in NORMAL when escape then it collapses to the caret', function()
    local s = h.freshSpec()
    s:given('word', '<caret>hello')
    s:whenKeys('w')
  ...
  it('given a pending find when escape then the pending key is dropped', function()
    ...
    s:given('word', '<caret>hello')
    s:whenKeys('f')
    h.neq(s.state.pending, nil)
    s:pressEsc()
    h.eq(s.state.pending, nil)
    s:whenKeys('l')
    s:thenCaretAt(1)
  end)
