...
ctx = require('neomeow.adapter').contextFor(buf)
check('ESC returned to NORMAL', ctx.state.mode == 'NORMAL')
...
feed('<C-e>')
ctx = require('neomeow.adapter').contextFor(buf)
check('the C-e chord ran move-end-of-line', ctx.port:getSelections()[1].active == 11)

buf = freshBuf({ 'hello world' })
feed('<M-f>')
ctx = require('neomeow.adapter').contextFor(buf)
check('the M-f chord ran forward-word', ctx.port:getSelections()[1].active == 5)

buf = freshBuf({ 'hello world' })
feed('i<C-e>')
ctx = require('neomeow.adapter').contextFor(buf)
check('chords stay out of INSERT', ctx.port:getSelections()[1].active ~= 11)
feed('<Esc>')

buf = freshBuf({ 'alpha' })
feed('i<M-;>')
ctx = require('neomeow.adapter').contextFor(buf)
check('Alt+; from INSERT opened the keypad', ctx.state.mode == 'KEYPAD')
check('the keypad remembers it came from INSERT', ctx.state.keypadPreviousState == 'INSERT')
feed('<Esc>')

buf = freshBuf({ 'editor' })
local editorWin = vim.api.nvim_get_current_win()
vim.cmd('split')
local toolWin = vim.api.nvim_get_current_win()
local toolBuf = vim.api.nvim_create_buf(false, true)
vim.api.nvim_win_set_buf(toolWin, toolBuf)
check('a scratch tool window is not a meow editor', require('neomeow.adapter').contextFor(toolBuf) == nil)
vim.api.nvim_set_current_win(editorWin)
vim.api.nvim_set_current_win(toolWin)
feed('<Esc>')
check('one escape in a tool window stays put', vim.api.nvim_get_current_win() == toolWin)
feed('<Esc>')
check('a second escape jumps back to the last editor', vim.api.nvim_get_current_win() == editorWin)
vim.api.nvim_win_close(toolWin, true)
