...
describe('WindmoveSpec', function()
  ...
  it('given the bundled rc then SPC w r fires the ace-resize hint command', function()
    local s = h.freshSpec()
    h.eq(Rc.defaults().keypad['wr'].command, 'ace-resize')
    h.eq(Rc.defaults().keypad['wr'].keys, nil)
    s:given('two lines', '<caret>one\ntwo')
    s:whenKeys(' wr')
    h.eq(s.ui.hints[1], 'ace-resize is not wired yet')
    s:thenMode(h.MeowMode.NORMAL)
  end)
end)
